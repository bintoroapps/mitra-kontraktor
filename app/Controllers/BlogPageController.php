...
class BlogPageController extends BaseController {
    ...
    public function index() {
        $db = db_connect();
        $model = new BlogPageModel($db);
        $data['blog'] = $model->getBlog();
        ...
        if($this->request->getMethod() == 'post') {
            $act = $this->request->getVar('act');
             if($act == '1_background') {
                ...
                return redirect()->to('/admin/tampilan-website/blog');
            }
        }

        return view('admin/blog_page', $data);
    }
        ...
        if($img = $image) {
            if ($img->isValid() && ! $img->hasMoved()) {
                ...
                if($save == 'save-as-webp') {
                    ...
                    if($type == 'background'):
                        $model->updateBlog(['blog_page_' . $section . '_img' => $img_new]);
                        session()->setFlashdata('success', 'Background successfully updated');
                    else:
                        $model->updateBLog(['blog_page_' . $section . '_img_' . $position => $img_new]);
                    ...
                    if($type == 'background'):
                        $model->updateBlog(['blog_page_' . $section . '_img' => $img->getName()]);
                        session()->setFlashdata('success', 'Background successfully updated');
                    else:
                        $model->updateBLog(['blog_page_' . $section . '_img_' . $position => $img->getName()]);
                        session()->setFlashdata('success', 'Image successfully updated');
                    endif;
                }
            }
    ...
    public function update() {
        ...
        if($act == '1-background') {
            ...
            $model->updateBlog(['blog_page_1_img' => $image]);
            return 'true';
        } else if($act == '1-title') {
            $small_title = $this->request->getVar('small_title');
            $big_title = $this->request->getVar('big_title');
            $model->updateBlog(['blog_page_1_big_title' => $big_title, 'blog_page_1_small_title' => $small_title]);
            return 'true';
        } else if($act == '1-bread') {
            $bread_1 = $this->request->getVar('bread_1');
            $bread_2 = $this->request->getVar('bread_2');
            $bread_link = $this->request->getVar('bread_link');
            $model->updateBlog(['blog_page_1_bread_1' => $bread_1, 'blog_page_1_bread_2' => $bread_2, 'blog_page_1_bread_link' => $bread_link]);
            return 'true';
        }
    }
    ...
    public function getEditBackground() {
        ...
        return view('admin/blog-get-edit-background', $data);
    }
